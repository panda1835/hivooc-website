...
function extractFeaturedImage(post: WPTour): string | null {
...
async function getTourBySlug(slug: string): Promise<WPTour> {
  ...
  const res = await fetch(
    `${baseUrl}/wp-json/wp/v2/hivooc-tour?slug=${encodeURIComponent(slug)}&_embed`,
    { next: { revalidate: 300 } },
  );
...
async function getRelatedShortTrips(currentId: number): Promise<ShortTrip[]> {
  ...
  const baseUrl = WORDPRESS_BASE_URL.replace(/\/$/, "");
  const res = await fetch(`${baseUrl}/wp-json/wp/v2/hivooc-tour?per_page=100&_embed`, {
    next: { revalidate: 300 },
  });
